...
export const WORK: CaseStudy[] = [
  {
    ...
    sections: [
      ...
      {
        heading: "Built for the floor, not the desk",
        body: "It installs as a Progressive Web App, because the people who need it are walking the site rather than sitting at a workstation. The service worker caches the app shell only — deliberately. Live telemetry always comes from the network, because a cached energy reading is worse than no reading: it looks current and isn't.",
      },
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: "What I built",
        body: "A retrieval-augmented assistant that answers only from a fixed corpus and shows its working. Every answer carries the document and section it came from, plus the raw retrieved passages in a “show your work” panel — so you can verify that retrieval actually happened rather than trusting that it did. Follow-up questions keep conversation context: ask “do you offer stretch denim?” then “what fits is it suited for?” and it resolves the pronoun from history.",
      },
      {
        heading: "Refusing to answer is a feature",
        body: "The anti-hallucination path is the part I care about most. Ask it who the CEO is — something genuinely not in the corpus — and it says it doesn't know and routes you to the sales team, rather than assembling a confident guess from adjacent text. That behaviour is pinned by a test, not by hope.",
      },
      {
        heading: "Measured, not asserted",
        body: "There's a fixed evaluation harness that runs live against the API: five in-corpus factual questions checked for the correct answer and the correct cited source, one out-of-corpus question checked for a refusal, and one multi-turn check proving history is actually used. Current result is 7/7. The harness matters more than the score — it means a regression is visible instead of being discovered by a customer.",
      },
      {
        heading: "It runs on nothing",
        body: "Entirely local, at zero API cost: MiniLM embeddings via ONNX on CPU, Chroma for the vector store, and Llama 3.2 3B through Ollama for generation. That's a real constraint rather than a flex — a 3B model needs a tighter prompt and better retrieval than a frontier model would, so the retrieval quality has to carry more of the weight.",
      },
    ],
  },
  {
